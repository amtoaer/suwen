...
pub(crate) async fn auth(
    ...
    if let Some(jwt_token) = jar.get("jwt") {
        ...
        if user.id == 1 {
            req.extensions_mut().insert(Identity::Admin { me: user, identity });
        } else {
            req.extensions_mut()
        ...
        let uuid =
            Uuid::parse_str(anonymous_id.value()).map_err(|_| ApiError::bad_request("Invalid anonymous ID format"))?;
        let identity = suwen_entity::identity::Entity::find()
            .filter(suwen_entity::identity::Column::Uuid.eq(uuid))
            .one(&conn)
            .await?;
        req.extensions_mut().insert(Identity::Anonymous { uuid, identity });
    } else {
